...
class Coraal:
    def __init__(self, 
        ...
        self.path_to_metadata = os.path.join(self.path_to_coraal, "metadata.csv")
        self.sr = inference_config.sample_rate
        self.max_audio_length = dataset_config.max_audio_length

    def build_dataset(self):
        dataset = pd.read_csv(self.path_to_metadata)

        ### Remove audios Longer than self.max_audio_length Seconds ###
        dataset = dataset.loc[(dataset["end_frame"] - dataset["start_frame"])/self.sr <= self.max_audio_length]

        dataset = dataset[["transcription", "path_to_audio_split", "file_id", "audio_id", 
                           "gender", "age", "occupation", "location", "accent"]]

        ### Dataloader will use "path_to_audio" as column name ###
        dataset = dataset.rename(columns={"path_to_audio_split": "path_to_audio"})
        
        return dataset



class CoraalLoadToMem:
    def __init__(self, 
                 dataset_config=DatasetConfig,
                 inference_config=InferenceConfig):
...
class SantaBarbara:
    ...
    def build_dataset(self):
        ...
        metadata = pd.read_csv(os.path.join(self.path_to_root, "metadata.csv"))
        metadata = metadata.loc[metadata["duration"] <= self.max_audio_length].reset_index(drop=True)
...
class SupportedDatasets:
    ...
    supported_dataset: tuple = (Coraal, SpeechAccentArchive, \
                               Edacc, L2Arctic, Mozilla, SantaBarbara)
    


if __name__ == "__main__":
    data = Coraal().build_dataset()