...
from config import InferenceConfig, DatasetConfig
from audio_datasets import Coraal, SpeechAccentArchive, Edacc, L2Arctic, Mozilla, SantaBarbara
...
class BatchPreparation:
    ...
    def build_dataset(self, limit_audio_num=None):
        if isinstance(self.dataset_class, (Edacc)):
            self.pre_batch_data(limit_audio_num=limit_audio_num)
        elif isinstance(self.dataset_class, (Coraal, SpeechAccentArchive, L2Arctic, SantaBarbara)):
            self.realtime_load()
    ...
    def build_dataloader(self, batch_size):
        if isinstance(self.dataset_class, (Coraal, Edacc, SpeechAccentArchive, L2Arctic, SantaBarbara)):
    ...
    def _mozilla_collate_function(self, examples):
        ...
        """
        for e in examples:
            ### Grab Array from Audio and cast to float32 to ensure functionality with Accelerate AutoCasting to FP16 ###
            e["audio"] = e["audio"]["array"].astype(np.float32)
